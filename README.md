...
- Scheduling soft real-time con algoritmo EDF
...
Il sistema utilizza l'algoritmo di scheduling EDF (Earliest Deadline First) per gestire task periodici, fornendo garanzie temporali sulla loro esecuzione. Il sistema è da considerarsi **soft real-time**, poiché Linux non offre garanzie temporali sufficientemente robuste da consentire scheduling **hard real-time** o **firm real-time**. Tuttavia, dalle misurazioni empiriche dei tempi di esecuzione è emerso che i task completavano ampiamente prima delle rispettive deadline, fornendo quindi garanzie temporali molto solide.
